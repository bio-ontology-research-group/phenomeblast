...
ont.getClassesInSignature().each { cl ->
  if (cl.toString().indexOf("0010771")>-1) {
    dermaclass = cl
  }
}

def dermaset = new TreeSet()
reasoner.getSubClasses(dermaclass, false).getFlattened().each { cl ->
...
new File("phenotypes.txt").splitEachLine("\t") { line ->
  ...
  if (line.size()>1) {
    if (dis in hannah) {
      def omim = dis
      disphenotypesall[omim] = new TreeSet()
      line[1..-1].each {
	if (it in dermaset) {
	  disphenotypesall[omim].add(it)
	}
      }
    }
  }
