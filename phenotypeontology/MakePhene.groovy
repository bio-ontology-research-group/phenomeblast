...
def rat = new File("rat/ratphenotypes.txt")
def dicty = new File("dicty/dicty-phenotypes.txt")
def omim = new File("omim/omimphenotypes.txt")
//def orphanet = new File("omim/orphanetphenotypes.txt")
def kristin = new File("kristin/kristinall.txt")
def sider = new File("sider/sider-phenomenet.txt")
...
/********************************** Create worm genes and genotypes ********************************/

def worm2eq = [:]
worm.splitEachLine("\t") {
  def name = it[0].trim()
  if (id2class[it[1]]) {
    if (worm2eq[name]==null) {
      worm2eq[name]=new TreeSet()
    }
    worm2eq[name].add(id2class[it[1]])
  }
}
...
/********************************** Create rat genes and genotypes ********************************/

def rat2eq = [:]
rat.splitEachLine("\t") {
  def name = it[0].trim()
  if (rat2eq[name]==null) {
    rat2eq[name]=new TreeSet()
  }
  def id = it[1].trim()
  if (id2class[id]!=null) {
    rat2eq[name].add(id2class[id])
  }
}
...
/********************************** Create slime mold genes and genotypes ********************************/

def dicty2eq = [:]
dicty.splitEachLine("\t") {
  def name = it[0].trim()
  if (dicty2eq[name]==null) {
    dicty2eq[name]=new TreeSet()
  }
  def id = it[1].trim()
  if (id2class[id]!=null) {
    dicty2eq[name].add(id2class[id])
  }
}

dicty2eq.keySet().each {
  def name = it
  def val = dicty2eq[it]
  if (val!=null) {
    def cl = null
...
/********************************** Create Overgrowth diseases ********************************/

def kristin2eq = [:]
kristin.splitEachLine("\t") {
  def name = it[0].trim()
  if (kristin2eq[name]==null) {
    kristin2eq[name]=new TreeSet()
  }
  def id = it[1].trim()
  if (id2class[id]!=null) {
    kristin2eq[name].add(id2class[id])
  }
}

kristin2eq.keySet().each {
  def name = it
  def val = kristin2eq[it]
  def cl = null
  if (val.size()>=2) {
    cl = factory.getOWLObjectIntersectionOf(val)
  } else {
...
/********************************** Create SIDER drug effects ********************************/

def sider2eq = [:]
sider.splitEachLine("\t") {
  def name = it[0].trim()
  if (sider2eq[name]==null) {
    sider2eq[name]=new TreeSet()
  }
  def id = it[1].trim()
  if (id2class[id]!=null) {
    sider2eq[name].add(id2class[id])
  }
}

sider2eq.each { name, val ->
  if (val.size()>0) {
    def cl = null
    if (val.size()>1) {
      cl = factory.getOWLObjectIntersectionOf(val)
