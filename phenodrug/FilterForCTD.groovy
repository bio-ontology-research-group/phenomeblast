...
def drugs = new File("data/ctd/CTD_chemicals.tsv")
def genes = new File("data/ctd/CTD_genes.tsv")
def diseases = new File("data/ctd/CTD_diseases.tsv")
def relations = new File("data/ctd/CTD_chem_gene_ixns.tsv")
...
drugs.splitEachLine("\t") { line ->
  if (!line[0].startsWith("ChemicalName")) {
    def id = line[1]
    def exp = new Expando()
    exp.name = line[0]
...
genes.splitEachLine("\t") { line ->
  if (!line[0].startsWith("GeneSymbol")) {
    def id = line[2]
    def exp = new Expando()
    exp.name = line[1]
    exp.symbol = line[0]
...
diseases.splitEachLine("\t") {line ->
  if (!line[0].startsWith("DiseaseName")) {
    def id = line[1]
    def exp = new Expando()
    exp.name = line[0]
    exp.mappings = []
    line[2]?.split("|").each { exp.mappings << it }
    diseasemap[id]=exp
  }
}
...
relations.splitEachLine("\t") {line ->
  if (!line[0].startsWith("ChemicalName")) {
    def id1 = "MESH:"+line[1] // chemical id
    def id2 = line[4] // gene id
    def e1 = genemap[id2]
    def e2 = drugmap[id1]
    if (e1.drugs==null) {
