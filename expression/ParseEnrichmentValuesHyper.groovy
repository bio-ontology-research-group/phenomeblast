...
map.each { instance, exp ->
  try {
    new File("enrichment/$instance/groups.txt").splitEachLine("\t") { line ->
      if (! line[0].startsWith("root_node")) {
	def name = line[1]
	def id = line[2]
	def pval_high = new Double(line[8])
	def pval_low = new Double(line[7])
...
instance2pval.each { instance, set ->
  ...
  set.each { exp2 ->
    if (exp2.high < CUTOFF) {
      println "$instance\t${exp1.name}\t${exp1.concentration}\t${exp1.duration}\t${exp2.name}\t${exp2.id}\t${exp2.high}"
    }
  }
}