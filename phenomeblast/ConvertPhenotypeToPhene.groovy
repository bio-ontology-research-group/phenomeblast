...
def onturirel = "http://purl.obolibrary.org/obo/PHENE_"


OWLOntologyManager manager = OWLManager.createOWLOntologyManager();
OWLDataFactory fac = manager.getOWLDataFactory()

def po = fac.getOWLObjectProperty(IRI.create(onturirel+"part-of"))
def hp = fac.getOWLObjectProperty(IRI.create(onturirel+"has-part"))
def inheresin = fac.getOWLObjectProperty(IRI.create(onturirel+"inheres-in"))
def hasquality = fac.getOWLObjectProperty(IRI.create(onturirel+"has-quality"))
def qualifier = fac.getOWLObjectProperty(IRI.create(onturirel+"qualifier"))
def towards = fac.getOWLObjectProperty(IRI.create(onturirel+"towards"))
def pheneof = fac.getOWLObjectProperty(IRI.create(onturirel+"phene-of"))
...
list.each { term ->
  ...
  if ((term.quality == null) && (term.rels.size()==0) && (term.inter.size()==0)) {
    term.isAnatomy = true
  }
}
...
list.each { term ->
  if (term.isAnatomy) {
    ...
    term.rels.each {
      ...
      def op = fac.getOWLObjectProperty(IRI.create(onturirel+rn))
...
manager.saveOntology(ont, IRI.create("file:"+args[1]))
