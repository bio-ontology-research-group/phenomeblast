...
int main (int argc, char *argv[]) {
  ...
  /* now calculate the mean similarity and mean square similarity */
  double mean = 0.0 ;
  double squaremean = 0.0 ;
  for (int i = 0 ; i < size ; i++) {
    for (int j = 0 ; j < size ; j++) {
      mean += dist[i][j] ;
      squaremean += dist[i][j]*dist[i][j] ;
    }
  }
  mean = mean / (size*size) ;
  squaremean = squaremean / (size*size) ;

  double sigma = sqrt( squaremean - mean * mean ) ;

  for (int i = 0 ; i < size ; i++) {
    for (int j = 0 ; j < size ; j++) {
      dist[i][j] = (dist[i][j] - mean) / sigma ;
    }
  }

  ofstream fout("../data/c-all-twosided-normalized.txt");
  for (int i = 0 ; i < size ; i++) {
    for (int j = 0 ; j < size ; j++) {
      fout << dist[i][j] << "\t";
    }
  ...
  fout.flush();
  fout.close();
  fout.open("../data/c-all-twosided-normalized-phenotypes.txt");
