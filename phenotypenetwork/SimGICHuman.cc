...
int main (int argc, char *argv[]) {
  ifstream in("../data/human-phenotypes.txt");
  ...
  ifstream icin("../data/human-phenotypes-info.txt");
  while(icin) {
  ...
  ofstream fout("../data/human-c-all.txt");
  for (int i = 0 ; i < size ; i++) {
  ...
  fout.flush();
  fout.close();
  fout.open("../data/human-c-all-phenotypes.txt");
