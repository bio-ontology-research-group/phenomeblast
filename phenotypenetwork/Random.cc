...
inline float random_sim(unordered_set<string> s1, unordered_set<string> s2, string p1, string p2) {
  float max = 0.0f;
...
void computeline(int line) { // processes a single line of dist
  ...
  while (it != phenotypes.end()) {
    string p2 = (*it).first;
    dist[line][counter] = random_sim(phenos, (*it).second, p1, p2) ;
    counter++;
    it++;
  }
...
int main (int argc, char *argv[]) {
  ...
  ofstream fout("../data/c-all-random.txt");
  for (int i = 0 ; i < size ; i++) {
  ...
  fout.flush();
  fout.close();
  fout.open("../data/c-all-phenotypes-random.txt");
