...
infile.splitEachLine("\t") { line ->
  ...
  if (a2!=null || second.startsWith("OMIM")) {
	...
	if (a1!=null || a2!=null) {
	  ...
	  if (diseasemap[dis]==null) {
	    diseasemap[dis] = new TreeSet()
	  }
	  if (diseasehitmap[dis]==null) {
	    diseasehitmap[dis] = new TreeSet()
	  }
	  diseasemap[dis] << (1-val)
	  if (x2disease[dis]!=null) {
	    def genes = x2disease[dis]
	    genes = genes.collect { mmap[it] }.flatten()
	    if (kid.intersect(genes).size()>0) {
	      disease << val
	      diseasehitmap[dis] << (1-val)
	    }
	  }
	}
      }
...
def rank = []
diseasemap.keySet().each { key ->
  def ref = diseasemap[key]
  def hit = diseasehitmap[key]
  def tempdal = new DoubleArrayList()
  ref.each { tempdal.add(it) }
  tempdal.sort()
  hit.each {
    rank << Descriptive.rankInterpolated(tempdal, it)
  }
}
rank.each {
  println it
