...
def minphenotypes = 3 // minimum number of directly annotated phenotypes
def mininfphenotypes = 7 // minimum number of inferred phenotypes

def fn = args[0] // ontology filename

def fout = new PrintWriter(new BufferedWriter(new FileWriter(new File(args[1]))))

def mpfile = new File("../phenotypeontology/obo/human-phenotype-ontology.obo")
def mid = ""
...
ont.getClassesInSignature().each { cl ->
  ...
  { 
	...
	s.each {
	  ...
	  if (clname.indexOf("HP_")>-1 && (! (onn in obsolete))) {
