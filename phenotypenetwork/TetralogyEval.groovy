...
infile.splitEachLine("\t") { line ->
  ...
  //  }
  def a2 = mmap[second]
  if (a2!=null || (second.indexOf("OMIM_187500")>-1)) {
    def orthologs = []
    def pathways = []
    for (int col = 0 ; col < line.size() ; col++) {
      ...
      if ((a1!=null) || (first.indexOf("OMIM_187500")>-1)) {
	if ((second.indexOf("OMIM_187500")>-1)) {
	  def temp = first
	  first = second
	  second = temp
	}
	if ((first.indexOf("187500")>-1) && (mmap[second]!=null) && (second.indexOf("MGI")>-1)) {
