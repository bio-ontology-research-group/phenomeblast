...
new File("modelphenotypes/phenoGeneCleanData_fish.txt").splitEachLine("\t") { line ->
  ...
  if (e && q) {
    ...
    if (! (str in e2q2cl.keySet())) {
      edoneq[e].add(q)
      e2q2cl[str].add(C("PHENO:$counter"))
      q = and(q, some(R("has-modifier"),fac.getOWLClass(IRI.create("http://purl.obolibrary.org/obo/PATO_0000460"))))
      manager.addAxiom(outont, equiv(C("PHENO:$counter"), some(R("has-part"),and(e, some(R("has-quality"),q)))))
      counter += 1
    }
    def fcl = e2q2cl[str]
...
map.each { cl, exp ->
  exp.e.each { e ->
    if (e && ! (e in done)) {
      manager.addAxiom(outont, equiv(C("PHENO:$counter"), some(R("has-part"),and(some(R("part-of"),e), some(R("has-quality"),id2class["PATO:0000001"])))))
      // if (id2name[e]) {
      // 	println "Adding annotation to $e"
      // 	def an = fac.getOWLAnnotation(fac.getRDFSLabel(), fac.getOWLLiteral(id2name[e].toString()))
      // 	def axiom = fac.getOWLAnnotationAssertionAxiom(C("PHENO:$counter"), an)
      // 	manager.addAxiom(outont,axiom)
      // }
      counter += 1
      done.add(e)
...
manager.saveOntology(outont, IRI.create("file:/tmp/a.owl"))
